...
class HMM {
  constructor({ states, dimensions, seed }) {
    ...
    this._pi = tf.variable(
      ...
      false, `pi-${tf.Variable.nextVarId++}`, 'float32'
    )

    this._A = tf.variable(
      tf.zeros([this._states, this._states]),
      false, `A-${tf.Variable.nextVarId++}`, 'float32'
    )
  ...
  logLikelihood(data) {
    ...
    return tf.tidy('hmm-log-likelihood', () => {
      ...
      // alpha: (T, N, S), c: (T, N)
      const [alpha, c] = this._forward(pdf);
