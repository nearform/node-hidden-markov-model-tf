...
class HMM {
  constructor ({ states, dimensions, seed }) {
    ...
    // We need an engine, this has the side effect of making sure
    // there is one.
    tf.getBackend();

    const engine = tf.ENV.globalEngine;

    // Prior properbility (\pi) and transfer properbility (A)
    this._pi = tf.variable(
      tf.zeros([this._states]),
      false, `pi-${engine.nextVariableId()}`, 'float32'
    );

    this._A = tf.variable(
      tf.zeros([this._states, this._states]),
      false, `A-${engine.nextVariableId()}`, 'float32'
    );
